  ...
  private drawGrid(
    ...
    if (this.gridService.isPeyote(gridType)) {
      ...
      for (let col = 0; col < peyoteSubCols; col++) {
        ...
        // Even visual cols are UP/unshifted (no Y offset); odd visual cols are DOWN/shifted.
        const offsetY = isOddCol ? beadSize.height / 2 : 0;
        // Even visual cols map to even buffer rows → Math.ceil(bufHeight/2) beads.
        // Odd  visual cols map to odd  buffer rows → Math.floor(bufHeight/2) beads.
